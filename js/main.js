...
function displayDetail(arr) {
  ...
  for (let i = 1; i <= 20; i++) {
    if (arr[`strIngredient${i}`]) {
      ingredients += `<li class="bg-info-subtle rounded-2 p-2 me-3 mb-2 text-green">${
        arr[`strMeasure${i}`]
      ...
      <h2>instruction</h2>
        <p>${arr.strInstructions}</p>
          ...
          <div>
              <a href="${
                arr.strSource
              }" class="btn bg-green text-white text-capitalize" target="_blank"> Source</a>
              <a href="${
                arr.strYoutube
              }" class="btn bg-danger text-white text-capitalize"  target="_blank"> Youtube</a>
          </div>
    </div>
  ...
  <form class="w-75 mx-auto">
      <div class="row g-4">
          ...
          <div class="col-lg-6">
              ...
              <p id="alertPhone"
                  class="d-none form-control my-2 py-3 text-center text-brown border-0 bg-pink">
                  Enter valid Phone Number [+2/002**********]</p>
          </div>
          <div class="col-lg-6">
          ...
          <button id="btnSubmit" type="button onclick="getCategory()"
...
function isRePasswordValid() {
  return (
    ...
    document.getElementById("uRePassword").value
  );
}
